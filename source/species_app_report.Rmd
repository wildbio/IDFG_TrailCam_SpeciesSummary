...
species_dat <- params$species_data
...
deploy_dat <- params$deploy_dat
gmu_df <- params$gmu_df
...
if(nrow(dat_map) <= 25){
  ...
  geom_point(data = deploy_dat, aes(x = long, y = lat),
...
p  +
  ...
  coord_quickmap() +
  #north(gmu_df) +
  #scalebar(gmu_df, dist = 100, dist_unit = "km",
  #         transform = T, model = "WGS84", st.size = 3, height = 0.01,
  #         anchor = c(x = -111, y = 41.8)) +
  theme_minimal() +
